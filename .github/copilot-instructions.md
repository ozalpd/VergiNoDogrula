...
│   │   └── TaxPayerCollectionVM.cs  # ObservableCollection<TaxPayerVM> + commands
│   ├── Resources/
...
A .NET class library containing all validation rules and data models. **No WPF or UI dependencies.**
...
| `ViewModels/TaxPayerCollectionVM.cs` | Owns `ObservableCollection<TaxPayerVM>`, `SelectedItem`, and `AddTaxPayerCommand`. |
| `Commands/AbstractCommand.cs` | Base `ICommand` with `RaiseCanExecuteChanged`. |
...
| `MainWindow.xaml` | Grid layout with labelled TextBoxes bound to `SelectedItem.TaxNumber` / `SelectedItem.Title`, an "Ekle" (Add) button, and a `DataGrid`. |
...
- `DataContext` is assigned in code-behind constructors (e.g., `MainWindow`). No DI container is currently used.
...
1. **New model property** → Add to `ITaxPayer`, implement with setter validation in `TaxPayer`, expose in `TaxPayerVM` with `try/catch` validation, bind in XAML.
2. **New command** → Inherit from `AbstractCommand`, register in the appropriate collection ViewModel, bind via `Command` / `CommandParameter` in XAML.
